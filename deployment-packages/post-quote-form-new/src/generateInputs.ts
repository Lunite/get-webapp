import { ReqBody } from "."
import * as ex from "exceljs"

export interface Inputs {
  electricityDemand: number
  priceOfElectricity: number
  standingCharge: number
  annualCostOfElectricity: number
  quantityOfPanels: number
  panelManufacturer: string
  panelWattage: number
  systemSize: number
  specificYield: number
  shadingFactor: number
  annualYield: number
  irradienceZone: string
  roofPitch: number
  azimuth: number
  roofType: string
  panels: string
  scaffoldRequired: boolean
  storageSize: number
  postcodeShort: string
  salesmanDiscount: number
  mainMargin: number
  finalMargin: number
  VATLevel: number
  additionalItems: { [k: string]: number }
}

export const generateInputs = (
  body: ReqBody,
  workbook: ex.Workbook,
  isCommercial: boolean
): Inputs => {
  const electricityDemand: number =
    body.eac === -1 ? calculateEAC(body, isCommercial) : body.eac
  const panelsAndBattery = calculatePanelsAndBattery(electricityDemand)
  const quantityOfPanels: number = panelsAndBattery.panels
  const storageSize: number = panelsAndBattery.battery
  const panelWattage: number = 330
  const [irradienceZone, postcodeShort] = getIrradienceZone(body.postcode)
  const mainMargin = isCommercial ? 0.25 : 0.8
  const inputs: Inputs = {
    electricityDemand,
    priceOfElectricity: body.ppw === -1 ? 0.1756 : body.ppw / 100,
    standingCharge:
      body.standingCharge === -1 ? 0.2226 : body.standingCharge / 100,
    annualCostOfElectricity:
      (electricityDemand * body.ppw) / 100 + (body.standingCharge / 100) * 365,
    quantityOfPanels,
    panelManufacturer: "Phonosolar", // Taken from old code as hard coded default
    panelWattage, // Taken from old code as hard coded default
    systemSize: (quantityOfPanels * panelWattage) / 1000,
    specificYield: 0, // This gets calculated after
    shadingFactor: 0.95, //Hard coded
    annualYield: 0, // This gets calculated after
    irradienceZone,
    roofPitch: body.roof.inclination !== -1 ? body.roof.inclination : 30,
    azimuth: Math.abs(Math.round(body.roof.azimuth / 5) * 5),
    roofType: body.roof.roofMaterial || "concreteRoof",
    panels: "black",
    scaffoldRequired: false, //Hard Coded
    storageSize,
    postcodeShort,
    mainMargin,
    VATLevel: isCommercial ? 0.2 : 0.05,
    finalMargin: mainMargin - (body.discount || 0),
    salesmanDiscount: body.discount || 0,
    additionalItems: {},
  }
  inputs.specificYield = getSpecificYield(
    irradienceZone,
    inputs.roofPitch,
    inputs.azimuth,
    workbook
  )
  inputs.annualYield =
    inputs.specificYield *
    inputs.systemSize *
    (isCommercial ? 1 : inputs.shadingFactor)
  inputs.additionalItems = getAdditionalCosts(inputs.systemSize)
  return inputs
}

const calculatePanelsAndBattery = (
  eac: number
): { panels: number; battery: number } => {
  switch (true) {
    case eac <= 2000:
      return {
        panels: 8,
        battery: 2.5,
      }
    case eac <= 3000:
      return {
        panels: 10,
        battery: 2.5,
      }
    case eac <= 4000:
      return {
        panels: 12,
        battery: 5,
      }
    case eac <= 5000:
      return {
        panels: 14,
        battery: 5,
      }
    case eac <= 6000:
      return {
        panels: 16,
        battery: 5,
      }
    case eac <= 7000:
      return {
        panels: 18,
        battery: 5,
      }
    case eac <= 8000:
      return {
        panels: 20,
        battery: 7.5,
      }
    case eac <= 9000:
      return {
        panels: 22,
        battery: 7.5,
      }
    case eac <= 10000:
      return {
        panels: 24,
        battery: 7.5,
      }
    case eac <= 11000:
      return {
        panels: 26,
        battery: 10,
      }
    case eac <= 12000:
      return {
        panels: 28,
        battery: 10,
      }
    case eac <= 13000:
      return {
        panels: 30,
        battery: 10,
      }
    case eac <= 14000:
      return {
        panels: 32,
        battery: 10,
      }
    case eac <= 15000:
      return {
        panels: 34,
        battery: 12.5,
      }
    case eac <= 16000:
      return {
        panels: 36,
        battery: 12.5,
      }
    case eac <= 17000:
      return {
        panels: 38,
        battery: 12.5,
      }
    case eac <= 18000:
      return {
        panels: 40,
        battery: 15,
      }
    case eac <= 19000:
      return {
        panels: 42,
        battery: 15,
      }
    case eac <= 20000:
      return {
        panels: 44,
        battery: 15,
      }
    default:
      return {
        panels: 50,
        battery: 17.5,
      }
  }
}

const calculateEAC = (body: ReqBody, isCommercial: boolean) => {
  if (!isCommercial) {
    let runningTotal = bedsToEAC[body.property.bedrooms]?.std || 0
    // TODO add any additional items in here
    runningTotal += body.property.eCar ? 2500 : 0
    runningTotal += body.property.eHeater ? 3000 : 0
    runningTotal += body.property.heater ? 3000 : 0
    runningTotal += body.property.hotTub ? 3000 : 0
    runningTotal += body.property.pool ? 3500 : 0
    runningTotal += body.property.pump ? 3000 : 0

    return runningTotal
  } else {
    // TODO what should the eac be for commercial if they are unsure
    return 45000
  }
}

const getSpecificYield = (
  zone,
  pitch,
  azimuth,
  workbook: ex.Workbook
): number => {
  // gets specific yield from spreadsheet
  const table = workbook.getWorksheet("MCS Irradiation Zones")
  let irradienceIdx
  let pitchIdx
  let azimuthIdx
  table.eachRow((row, rowNum) => {
    if (!irradienceIdx) {
      if (row.getCell(1).value == zone) {
        irradienceIdx = rowNum
      }
    } else if (!pitchIdx) {
      if (row.getCell(2).value == pitch) {
        pitchIdx = rowNum
      }
    }
  })
  const azRow = table.getRow(2)
  azRow.eachCell((cell, idx) => {
    if (!azimuthIdx) {
      if (cell.value == azimuth) {
        azimuthIdx = idx
      }
    }
  })
  return Number(table.getCell(pitchIdx, azimuthIdx).value)
}

const bedsToEAC = {
  2: {
    low: 2000,
    std: 2500,
    high: 3500,
  },
  3: {
    low: 3000,
    std: 3500,
    high: 4000,
  },
  4: {
    low: 3500,
    std: 4000,
    high: 4500,
  },
  5: {
    low: 4000,
    std: 4500,
    high: 5000,
  },
  6: {
    low: 4500,
    std: 5000,
    high: 5500,
  },
}

const getIrradienceZone = (postcode): [string, string] => {
  // gets irradience zone from short postcode
  const irradienceLookup = {
    AL: "1",
    B: "6",
    BA: "5E",
    BB: "7E",
    BD: "11",
    BD23: "10",
    BD24: "10",
    BH: "3",
    BL: "7E",
    BN: "2",
    BR: "2",
    BS: "5E",
    BT: "21",
    CA: "8E",
    CB: "12",
    CF: "5W",
    CH: "7E",
    CH5: "7W",
    CH6: "7W",
    CH7: "7W",
    CH8: "7W",
    CM: "12",
    CM21: "1",
    CM22: "1",
    CM23: "1",
    CO: "12",
    CR: "1",
    CT: "2",
    CV: "6",
    CW: "7E",
    DA: "2",
    DD: "15",
    DE: "6",
    DG: "8S",
    DH: "10",
    DH4: "9E",
    DH5: "9E",
    DL: "10",
    DN: "11",
    DT: "2",
    DY: "6",
    E: "1",
    EC: "1",
    EH: "15",
    EH43: "9S",
    EH44: "9S",
    EH45: "9S",
    EH46: "9S",
    EN: "1",
    EN9: "12",
    EX: "4",
    FK: "14",
    FY: "7E",
    G: "14",
    GL: "5E",
    GU: "1",
    GU11: "3",
    GU12: "3",
    GU14: "3",
    GU28: "2",
    GU29: "2",
    GU30: "3",
    GU31: "3",
    GU32: "3",
    GU33: "3",
    GU34: "3",
    GU35: "3",
    GU46: "3",
    GU51: "3",
    GU52: "3",
    HA: "1",
    HD: "11",
    HG: "10",
    HP: "1",
    HR: "6",
    HS: "18",
    HU: "11",
    HX: "11",
    IG: "12",
    IP: "12",
    IV: "17",
    IV30: "16",
    IV31: "16",
    IV32: "16",
    IV36: "16",
    KA: "14",
    KT: "1",
    KW: "17",
    KW15: "19",
    KW16: "19",
    KW17: "19",
    KY: "15",
    L: "7E",
    LA: "7E",
    LA7: "8E",
    LA8: "8E",
    LA9: "8E",
    LA10: "8E",
    LA11: "8E",
    LA12: "8E",
    LA13: "8E",
    LA14: "8E",
    LA15: "8E",
    LA16: "8E",
    LA17: "8E",
    LA18: "8E",
    LA19: "8E",
    LA20: "8E",
    LA21: "8E",
    LA22: "8E",
    LA23: "8E",
    LD: "13",
    LE: "6",
    LL: "7W",
    LL23: "13",
    LL24: "13",
    LL25: "13",
    LL26: "13",
    LL27: "13",
    LL30: "13",
    LL31: "13",
    LL32: "13",
    LL33: "13",
    LL34: "13",
    LL35: "13",
    LL36: "13",
    LL37: "13",
    LL38: "13",
    LL39: "13",
    LL40: "13",
    LL41: "13",
    LL42: "13",
    LL43: "13",
    LL44: "13",
    LL45: "13",
    LL46: "13",
    LL47: "13",
    LL48: "13",
    LL49: "13",
    LL50: "13",
    LL51: "13",
    LL52: "13",
    LL53: "13",
    LL54: "13",
    LL55: "13",
    LL56: "13",
    LL57: "13",
    LL58: "13",
    LL59: "13",
    LL60: "13",
    LL61: "13",
    LL62: "13",
    LL63: "13",
    LL64: "13",
    LL65: "13",
    LL66: "13",
    LL67: "13",
    LL68: "13",
    LL69: "13",
    LL70: "13",
    LL71: "13",
    LL72: "13",
    LL73: "13",
    LL74: "13",
    LL75: "13",
    LL76: "13",
    LL77: "13",
    LL78: "13",
    LN: "11",
    LS: "11",
    LS24: "10",
    LU: "1",
    M: "7E",
    ME: "2",
    MK: "1",
    ML: "14",
    N: "1",
    NE: "9E",
    NG: "11",
    NN: "6",
    NP: "5W",
    NPS: "13",
    NR: "12",
    NW: "1",
    OL: "7E",
    OX: "1",
    PA: "14",
    PE: "12",
    PE9: "11",
    PE10: "11",
    PE11: "11",
    PE12: "11",
    PE20: "11",
    PE21: "11",
    PE22: "11",
    PE23: "11",
    PE24: "11",
    PE25: "11",
    PH: "15",
    PH19: "17",
    PH20: "17",
    PH21: "17",
    PH22: "17",
    PH23: "17",
    PH24: "17",
    PH25: "17",
    PH26: "16",
    PH30: "17",
    PH31: "17",
    PH32: "17",
    PH33: "17",
    PH34: "17",
    PH35: "17",
    PH36: "17",
    PH37: "17",
    PH38: "17",
    PH39: "17",
    PH40: "17",
    PH41: "17",
    PH42: "17",
    PH43: "17",
    PH44: "17",
    PH49: "14",
    PH50: "14",
    PL: "4",
    PO: "2",
    PO18: "2",
    PO19: "2",
    PO20: "2",
    PO21: "2",
    PO22: "2",
    PR: "7E",
    RG: "1",
    RG21: "3",
    RG22: "3",
    RG23: "3",
    RG24: "3",
    RG25: "3",
    RG26: "3",
    RG27: "3",
    RG28: "3",
    RG29: "3",
    RH: "1",
    RH10: "2",
    RH11: "2",
    RH12: "2",
    RH13: "2",
    RH14: "2",
    RH15: "2",
    RH16: "2",
    RH17: "2",
    RH18: "2",
    RH19: "2",
    RH20: "2",
    RH77: "2",
    RM: "12",
    S: "11",
    S18: "6",
    S32: "6",
    S33: "6",
    S40: "6",
    S41: "6",
    S42: "6",
    S43: "6",
    S44: "6",
    S45: "6",
    S49: "6",
    SA: "5W",
    SA14: "13",
    SA15: "13",
    SA16: "13",
    SA17: "13",
    SA18: "13",
    SA19: "13",
    SA20: "13",
    SA31: "13",
    SA32: "13",
    SA33: "13",
    SA34: "13",
    SA35: "13",
    SA36: "13",
    SA37: "13",
    SA38: "13",
    SA39: "13",
    SA40: "13",
    SA41: "13",
    SA42: "13",
    SA43: "13",
    SA44: "13",
    SA45: "13",
    SA46: "13",
    SA47: "13",
    SA48: "13",
    SA61: "13",
    SA62: "13",
    SA63: "13",
    SA64: "13",
    SA65: "13",
    SA66: "13",
    SA67: "13",
    SA68: "13",
    SA69: "13",
    SA70: "13",
    SA71: "13",
    SA72: "13",
    SA73: "13",
    SE: "1",
    SG: "1",
    SK: "7E",
    SK13: "6",
    SK17: "6",
    SK22: "6",
    SK23: "6",
    SL: "1",
    SM: "5E",
    SN7: "1",
    SO: "3",
    SP: "5E",
    SP6: "3",
    SP7: "3",
    SP8: "3",
    SP9: "3",
    SP10: "3",
    SP11: "3",
    SR: "9E",
    SR7: "10",
    SR8: "10",
    SS: "12",
    ST: "6",
    SW: "1",
    SY14: "7E",
    SY15: "13",
    SY16: "13",
    SY17: "13",
    SY18: "13",
    SY19: "13",
    SY20: "13",
    SY21: "13",
    SY22: "13",
    SY23: "13",
    SY24: "13",
    SY25: "13",
    TA: "5E",
    TD: "9S",
    TD12: "9E",
    TD15: "9E",
    TF: "6",
    TN: "2",
    TQ: "4",
    TR: "4",
    TS: "10",
    TW: "1",
    UB: "1",
    W: "1",
    WA: "7E",
    WC: "1",
    WD: "1",
    WF: "11",
    WN: "7E",
    WR: "6",
    WS: "6",
    WV: "6",
    YO: "10",
    YO15: "11",
    YO16: "11",
    YO25: "11",
    ZE: "20",
  }
  const codes = Object.keys(irradienceLookup)
  let code = postcode.substr(0, 4)
  if (codes.includes(code)) {
    return [irradienceLookup[code], code]
  }
  code = postcode.substr(0, 3)
  if (codes.includes(code)) {
    return [irradienceLookup[code], code]
  }
  code = postcode.substr(0, 2)
  if (codes.includes(code)) {
    return [irradienceLookup[code], code]
  }
  code = postcode.substr(0, 1)
  return [irradienceLookup[code], code]
}

const getAdditionalCosts = (systemSize: number): { [k: string]: number } => {
  let additionalItems = {}
  if (systemSize >= 5) {
    additionalItems = { ...additionalItems, "Grid Application 2": 650 }
  } else if (systemSize >= 4) {
    additionalItems = { ...additionalItems, "Grid Application": 160 }
  }
  if (systemSize >= 4 && systemSize <= 5) {
    additionalItems = {
      ...additionalItems,
      "Sofar Solar 4000 HYD Hybrid Inverter": 699.5 - 92 * systemSize,
    }
  } else if (systemSize >= 5 && systemSize <= 6) {
    additionalItems = {
      ...additionalItems,
      "Sofar Solar 5000 HYD Hybrid Inverter": 764.4 - 92 * systemSize,
    }
  } else if (systemSize >= 6) {
    additionalItems = {
      ...additionalItems,
      "Sofar Solar 6000 HYD Hybrid Inverter": 799.68 - 92 * systemSize,
    }
  }
  return additionalItems
}
