...
export const calculateCostOfSystem = (
  ...
  // scaffold cost
  runningTotal += scaffoldRequired
    ? calculateScaffolding(isCommercial, solarKw)
    : 0
  ...
  runningTotal += additionalItemCosts.reduce((a, b) => a + b, 0)

  // Calculate Margin
  const marginCost = runningTotal * margin
  const increasedMarginCost = runningTotal * 0.8 // High margin for PDF
...
const calculateAncillaryMaterialsCost = (
  solarKW: number,
  isCommercial: boolean
): number => {
...
const calculateSolarModuleColorCost = (
  color: string | number,
  solarKW: number
): number => {
